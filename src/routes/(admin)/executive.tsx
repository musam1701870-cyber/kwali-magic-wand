...
function KpiCard({
  ...
  return (
    <div className={`rounded-2xl bg-card p-5 shadow-[var(--shadow-card)] ${ring}`}>
      ...
      {hint && (
        <div
          className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${chip}`}
        >
          {hint}
        </div>
      )}
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-primary-foreground/85">
              Here is today's revenue performance summary for Kwali Area Council.
            </p>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <KpiCard
          label="Total Revenue"
          value={fmtNaira(kpis.year)}
          hint="Year to date"
          tone="positive"
        />
        <KpiCard
          label="Revenue This Month"
          value={fmtNaira(kpis.month)}
          hint="+8% MoM"
          tone="positive"
        />
        ...
        <KpiCard
          ...
          value={fmtNaira(kpis.outstanding)}
          hint={`${kpis.defaulters.toLocaleString()} defaulters`}
          tone="danger"
        />
        <KpiCard
        ...
        <KpiCard
          ...
          value={kpis.taxpayers.toLocaleString()}
          hint="Active records"
        />
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Monthly collection · January → December
              </p>
            </div>
            ...
            {monthlyTrend.map((m) => (
              <div key={m.m} className="group flex flex-1 flex-col items-center gap-2">
                <div className="relative w-full">
                  ...
                  <div
                    className="w-full rounded-t-lg bg-gradient-to-t from-primary to-primary/50 transition group-hover:from-gold group-hover:to-gold/60"
      ...
      <div className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Collections across the 10 wards of Kwali Area Council
            </p>
          </div>
          <Link to="/gis" className="text-sm font-semibold text-primary hover:underline">
            View ward map →
          </Link>
        </div>
          ...
          {wardRevenue.map((w) => (
            <div key={w.ward} className="rounded-xl border border-border bg-surface p-4">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${w.compliance >= 70 ? "bg-primary/10 text-primary" : w.compliance >= 50 ? "bg-gold/20 text-gold-foreground" : "bg-destructive/10 text-destructive"}`}
                >
                  {w.compliance}%
                </span>
              </div>
              <div className="mt-2 font-display text-xl font-bold text-ink">
                {fmtNaira(w.collected)}
              </div>
              <div className="mt-1 text-[11px] text-muted-foreground">
                of {fmtNaira(w.expected)} expected
              </div>
              <div className="mt-2 h-1.5 rounded-full bg-secondary">
                <div
                  className="h-1.5 rounded-full bg-primary"
                  style={{ width: `${Math.min(100, (w.collected / w.expected) * 100)}%` }}
                />
              </div>
            </div>
          ))}
...
{/* Row 4 — Market Intelligence */}
      <div className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
        <div className="mb-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Trader registration · attendance · daily collection · officer accountability
            </p>
          </div>
          <Link
            to="/markets"
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          {[
            ...
            {
              label: "Present Today",
              value: marketSessions.reduce((s, m) => s + m.present, 0),
              icon: <UserCheck className="h-4 w-4" />,
              color: "bg-blue-50 text-blue-700",
            },
            {
              label: "Paid Today",
              value: traders.filter((t) => t.paidToday).length,
            ...
            {
              label: "Revenue Today",
              value: fmtNaira(marketSessions.reduce((s, m) => s + m.revenue, 0)),
            ...
            return (
              <div key={s.id} className="rounded-xl border border-border bg-surface p-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${paidPct >= 80 ? "bg-emerald-50 text-emerald-700" : paidPct >= 60 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}
                  >
                    {paidPct}% paid
                  </span>
                </div>
        ...
        <div className="mt-4 overflow-x-auto">
          <div className="flex items-center gap-3 min-w-max">
            <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground w-28 shrink-0">
              Officer
            </div>
            {marketOfficers.map((o) => (
              <div
                key={o.id}
                className="flex items-center gap-2.5 rounded-xl border border-border bg-card px-3 py-2 min-w-[160px]"
              >
                ...
                <div>
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    {fmtNaira(o.collectedToday)} · {o.efficiency}%
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-4 overflow-x-auto">
          <div className="flex items-center gap-3 min-w-max">
            ...
            {marshals.map((m) => (
              ...
              >
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-primary/10 text-[10px] font-bold text-primary">
                  {m.name.split(" ").slice(-1)[0].slice(0, 2).toUpperCase()}
                </div>
                <div>
                  <div className="text-xs font-semibold text-ink">
                    {m.name.replace("Marshal ", "")}
                  </div>
                  <div className="text-[10px] text-muted-foreground">
                    {m.verificationsToday} verif. · {m.compliance}%
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-4 flex gap-3">
          ...
          <Link
            to="/markets/collect"
            className="rounded-lg border border-border bg-background px-4 py-2 text-sm font-semibold text-foreground transition:hover:bg-secondary"
          >
      ...
      {/* Row 5 — Financial Oversight */}
      <div className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
        <h2 className="font-display text-lg font-bold text-ink mb-4">Financial Oversight</h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl border border-border bg-surface p-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Remittance Rate</div>
            <div className="mt-2 font-display text-2xl font-bold text-ink">85.3%</div>
            <div className="text-[11px] text-muted-foreground">Remitted vs Collected</div>
          </div>
          <div className="rounded-xl border border-border bg-surface p-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Total Collected Today</div>
            <div className="mt-2 font-display text-2xl font-bold text-ink">₦2,450,000</div>
            <div className="text-[11px] text-muted-foreground">All channels</div>
          </div>
          <div className="rounded-xl border border-border bg-surface p-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Outstanding Variance</div>
            <div className="mt-2 font-display text-2xl font-bold text-destructive">₦610,000,000</div>
            <div className="text-[11px] text-muted-foreground">Year-to-date</div>
          </div>
          <div className="rounded-xl border border-border bg-surface p-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Marshal Compliance</div>
            <div className="mt-2 font-display text-2xl font-bold text-gold">93.6%</div>
            <div className="text-[11px] text-muted-foreground">Average across marshals</div>
          </div>
        </div>
      </div>
    </DashboardShell>
