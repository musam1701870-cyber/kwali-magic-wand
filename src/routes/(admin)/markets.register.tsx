...
import { markets, tradeTypes, marketZoneNames, wards, assetTypes } from "@/shared/lib/kwali-mock";
import type { AssetType } from "@/shared/lib/kwali-mock";
...
import { UserPlus, CheckCircle2, ArrowLeft, Download, QrCode, Copy, Users } from "lucide-react";

const FORM_STEPS = ["Personal", "Market & trade", "Emergency contact"];

export const Route = createFileRoute("/(admin)/markets/register")({
...
const blank: FormState = {
  ...
  zone: "",
  category: "C",
  assetType: "None / Market Trader Only",
  emergencyContact: "",
  emergencyPhone: "",
  passType: "Daily",
};
...
function TraderCard({
  ...
  return (
    <div
      className="overflow-hidden rounded-2xl border-2 border-primary shadow-[var(--shadow-elegant)]"
      style={{ width: 280 }}
    >
      {/* Card header */}
...
function RegisterTraderPage() {
  ...
  const subValid =
    sub === 0
      ? !!(form.name && form.phone && form.gender && form.ward)
      : sub === 1
        ? !!(form.marketId && form.tradeType && form.zone && form.category && form.assetType)
        : true;

  const next = () => setSub((s) => Math.min(s + 1, FORM_STEPS.length - 1));
  const back = () => setSub((s) => Math.max(s - 1, 0));

  const register = () => {
    if (!valid) return;
    const n = Math.floor(100000 + Math.random() * 899999);
    const id = `KWL-TRD-${String(n).padStart(6, "0")}`;
    setTraderId(id);
    setStep("success");
    toast.success("Trader registered", { description: `${form.name} · ${id}` });
  };

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    // Enter / submit on an earlier step advances instead of registering.
    if (sub < FORM_STEPS.length - 1) {
      if (subValid) next();
      return;
    }
    register();
      ...
      {step === "form" ? (
        <div className="mx-auto max-w-3xl">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-muted-foreground">
                  Register a new trader to generate their unique Trader ID and Digital Market ID
                  card. Collect only essential details — takes under 1 minute.
                </p>
              </div>
            </div>
          </div>
          ...
          <StepProgress
            ...
            current={sub}
            onJump={setSub}
            className="mb-6 rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)]"
          />
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <input
                        ...
                        placeholder="e.g. Pai, Yebu, Kilankwa"
                        className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none ring-primary/30 transition focus:ring-2"
                      />
                    </div>
                  </div>
              ...
              <>
                ...
                <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-ink">
                        Type of Goods Sold <span className="text-destructive">*</span>
                      </label>
                      <select
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-ink">
                        Usual Trading Zone <span className="text-destructive">*</span>
                      </label>
                      <select
                        value={form.zone}
                        onChange={set("zone")}
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-ink">
                        Trader Category <span className="text-destructive">*</span>
                      </label>
                      <select
                        value={form.category}
                      ...
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        A = Rice, Electronics, Building Materials · B = Fashion, Foodstuff · C =
                        Vegetables, Tomatoes
                      </p>
                    </div>
                    <div className="sm:col-span-2">
                      <label className="block text-sm font-semibold text-ink">
                        Assets / Business Type <span className="text-destructive">*</span>
                      </label>
                      <select
                      ...
                      <p className="mt-1 text-[11px] text-muted-foreground">
                        Physical/business assets owned or operated by the trader. Select "None" if
                        market trader only.
                      </p>
                    </div>
                    <div>
                ...
                {/* Optional trading-spot pin */}
                <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)]">
                  <h3 className="mb-1 font-display text-sm font-bold uppercase tracking-widest text-muted-foreground">
                    Usual trading spot{" "}
                  ...
                  <p className="mb-4 text-sm text-muted-foreground">
                    Pin where the trader usually sets up — helps officers find them on market days.
                  </p>
                  <LocationPicker value={spot} height={260} onChange={(v) => setSpot(v)} />
                </div>
              </>
            )}
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Next of kin or guarantor"
                        className="mt-1 w-full rounded-lg border border-border bg-background px-3 py-2.5 text-sm outline-none ring-primary/30 transition focus:ring-2"
                      />
                    </div>
                    <div>
        ...
        <div className="mx-auto max-w-2xl">
          <div className="mb-6 flex items-start gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
            ...
            <div>
              <h2 className="font-display text-lg font-bold text-emerald-800">
                Trader Registered Successfully!
              </h2>
              <p className="mt-1 text-sm text-emerald-700">
                <strong>{form.name}</strong> has been registered. Their Digital Market ID has been
                generated below. Print or share the QR card with the trader.
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="mt-1 text-sm text-muted-foreground">
                  {form.marketId ? selectedMarket?.name : ""} · {form.tradeType}
                </div>
              </div>
              <button
                onClick={copy}
            ...
            <button
              onClick={() =>
                void exportIdCardPDF({
                  ...
                  name: form.name,
                  lines: [
                    { label: "Market", value: selectedMarket?.name ?? "—" },
                    { label: "Trade", value: form.tradeType || "—" },
                    { label: "Category", value: `Cat ${form.category}` },
                    { label: "Ward", value: form.ward },
                    { label: "Pass", value: form.passType },
                  ],
                  footer: `KSRP · ${form.assetType}`,
                })
              }
              className="flex items-center gap-2 rounded-lg border border-border bg-background px-5 py-2.5 text-sm font-semibold text-foreground transition hover:bg-secondary"
            >
