...
import { vehicles } from "@/shared/lib/kwali-mock";
...
function TransportPage() {
  ...
  function issueTicket() {
    if (!plate.trim()) {
      toast.error("Enter a plate or vehicle number first.");
      return;
    }
    const ref = `KWL-${type.slice(0, 3).toUpperCase()}-2026-${String(Math.floor(Math.random() * 9000 + 1000))}`;
    setIssued({ ref, plate: plate.trim().toUpperCase(), type, days, amount: total });
    toast.success(`Ticket issued · ₦${total.toLocaleString()}`, {
      description: `${type} · ${plate.trim().toUpperCase()} · ${days} day${days > 1 ? "s" : ""}`,
    });
  }
  ...
  function printSticker() {
    if (!issued) return;
    exportReceiptPDF({
      filename: `transport-ticket-${issued.ref}`,
      receiptNo: issued.ref,
      payerName: `${issued.type} · ${issued.plate}`,
      lines: [
        { label: "Vehicle type", value: issued.type },
        { label: "Plate number", value: issued.plate },
        { label: "Validity", value: `${issued.days} day${issued.days > 1 ? "s" : ""}` },
        { label: "Status", value: "Compliant" },
      ],
      amount: `₦${issued.amount.toLocaleString()}`,
      note: "Present this QR sticker to enforcement officers on request.",
    });
  ...
  function sendSms() {
    if (!issued) return;
    const body = `Kwali Transport Ticket ${issued.ref} — ${issued.type} ${issued.plate}, valid ${issued.days} day(s). Amount ₦${issued.amount.toLocaleString()}. Compliant.`;
    // Opens the device SMS composer where available; falls back to a confirmation toast.
    window.location.href = `sms:?body=${encodeURIComponent(body)}`;
    toast.success("Opening SMS app…", { description: `Ticket ${issued.ref} ready to send.` });
  }

  return (
    <DashboardShell
      title="Transport ticketing"
      subtitle="Buy daily tickets, manage QR stickers and view active permits."
      requireAdmin={false}
    >
      <div className="mb-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-[var(--shadow-card)] lg:col-span-2">
          ...
          <p className="text-sm text-muted-foreground">
            Issues a QR-coded permit linked to the plate number.
          </p>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    {issued.type} · {issued.plate} · {issued.days} day{issued.days > 1 ? "s" : ""}
                  </div>
