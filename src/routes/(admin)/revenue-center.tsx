...
import { exportCSV, exportPDF, exportXLSX, type ExportRow } from "@/shared/lib/exporters";
...
export const Route = createFileRoute("/(admin)/revenue-center")({
...
function RevenueCenter() {
  ...
  const totals = useMemo(() => {
    const sum = (rows: ExportRow[], key: string) =>
      rows.reduce((s, r) => s + (Number(r[key]) || 0), 0);
    return {
      businesses: {
        count: datasets.businesses.length,
        value: sum(datasets.businesses, "Annual Levy"),
      },
      properties: {
        count: datasets.properties.length,
        value: sum(datasets.properties, "Annual Rate"),
      },
      ...
      taxpayers: {
        count: datasets.taxpayers.length,
        value: sum(datasets.taxpayers, "Outstanding"),
      },
  ...
  const grandTotal = useMemo(
    () =>
      totals.businesses.value +
      totals.properties.value +
      totals.markets.value +
      totals.payments.value,
    [totals],
  );
  ...
  const doExport = (kind: "csv" | "xlsx" | "pdf") => {
    ...
    if (kind === "pdf")
      exportPDF({
        filename: exportBase,
        title: `${CATEGORY_LABEL[category]} Report`,
  ...
  return (
    <DashboardShell
      ...
      subtitle="Search and export bills, payments, arrears and notices across every revenue stream"
      actions={
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={() => doExport("pdf")} disabled={!activeRows.length}>
        ...
        <CardContent>
          <div className="grid gap-3 md:grid-cols-5">
            ...
            <div>
              ...
              <Input
                placeholder="Name, ref, TIN, plate…"
                value={q}
      ...
      <Tabs value={category} onValueChange={(v) => setCategory(v as Category)} className="mt-6">
          ...
          {(Object.keys(CATEGORY_LABEL) as Category[]).map((c) => (
            ...
            >
              ...
              <Badge variant="secondary" className="ml-2">
                {c === "all" ? activeRows.length : datasets[c].length}
              </Badge>
            </TabsTrigger>
          ))}
        </TabsList>
        ...
        {(Object.keys(CATEGORY_LABEL) as Category[]).map((c) => (
          <TabsContent key={c} value={c} className="mt-4">
            <DataTable
              rows={c === "all" ? Object.values(datasets).flat() : datasets[c]}
              grandTotal={c === "all" ? grandTotal : undefined}
            />
          </TabsContent>
        ))}
      </Tabs>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
              ...
              <tr key={i} className="hover:bg-secondary/30">
                {cols.map((c) => (
                  <td key={c} className="px-4 py-2.5 text-ink">
                    {typeof r[c] === "number" &&
                    /amount|levy|rate|fee|outstanding|target|collected/i.test(c)
                      ? fmt(r[c] as number)
                      : String(r[c])}
                  </td>
                ))}
      ...
      <div className="flex items-center justify-between border-t border-border bg-secondary/30 px-4 py-2 text-xs text-muted-foreground">
        <span>
          Showing {Math.min(rows.length, 250)} of {rows.length} records
        </span>
        {grandTotal !== undefined && (
          <span className="font-semibold text-ink">Combined revenue: {fmt(grandTotal)}</span>
        )}
      </div>
    </div>
  );
