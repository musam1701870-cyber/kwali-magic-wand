...
import { wardRevenue, sourceMix, markets, monthlyTrend, marshals } from "@/shared/lib/kwali-mock";
...
function CommandCenterPage() {
  return (
    <DashboardShell
      ...
      subtitle="Real-time monitoring across wards, sources and officers"
    >
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Marshal performance">
          <ul className="divide-y divide-border">
            {marshals.map((m) => (
              <li key={m.id} className="flex items-center justify-between py-3 text-sm">
                <div>
                  <div className="font-semibold text-ink">{m.name}</div>
                  <div className="text-xs text-muted-foreground">Ward {m.ward}</div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-primary">{fmtNaira(m.remittanceToday)}</div>
                  <div className="text-[11px] text-muted-foreground">{m.verificationsToday} verifications</div>
                </div>
              </li>
            ))}
          </ul>
        ...
        <div className="flex h-40 items-end gap-3">
          {monthlyTrend.map((m) => (
            <div key={m.m} className="flex flex-1 flex-col items-center gap-1">
              <div
                className="w-full rounded-t bg-gradient-to-t from-primary to-gold"
                style={{ height: `${(m.v / 11000) * 100}%` }}
              />
