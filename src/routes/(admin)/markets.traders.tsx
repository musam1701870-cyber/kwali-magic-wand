...
import { traders, markets, type Trader } from "@/shared/lib/kwali-mock";
import {
  Search,
  UserPlus,
  Filter,
...
export const Route = createFileRoute("/(admin)/markets/traders")({
...
function ComplianceBadge({ score }: { score: number }) {
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${color}`}>{score}%</span>
  );
}
...
function CategoryBadge({ cat }: { cat: Trader["category"] }) {
  ...
  return (
    <span className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${map[cat]}`}>
      Cat {cat}
    </span>
  );
}
...
function TraderRow({ t, onClick }: { t: Trader; onClick: () => void }) {
  return (
    <tr onClick={onClick} className="cursor-pointer transition hover:bg-secondary/40">
      ...
      <td className="hidden px-4 py-3.5 text-sm text-muted-foreground lg:table-cell">
        {t.assetType !== "None / Market Trader Only" ? t.assetType : "—"}
      </td>
      ...
      <td className="hidden px-4 py-3.5 xl:table-cell">
        <ComplianceBadge score={t.complianceScore} />
      </td>
...
function printIdCard(t: Trader) {
  exportIdCardPDF({
    ...
    name: t.name,
    lines: [
      { label: "Market", value: t.marketName },
      { label: "Trade", value: t.tradeType },
      { label: "Category", value: `Cat ${t.category}` },
      { label: "Ward", value: t.ward },
      { label: "Pass", value: t.passType },
    ],
    footer: `KSRP · Daily ₦${t.dailyRate} · Registered ${t.registeredAt}`,
  });
  toast.success("ID card downloaded", { description: t.traderId });
}

function TraderDrawer({
  t,
  onClose,
  onCollect,
}: {
    ...
    <div className="fixed inset-0 z-50 flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="flex gap-2 border-b border-border px-5 py-3">
          ...
          <CategoryBadge cat={t.category} />
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${t.status === "Active" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
          >
            {t.status}
          </span>
        </div>
        ...
        <div className="border-t border-border p-4 space-y-2">
          {!t.paidToday && (
            <button
              onClick={() => onCollect(t)}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2.5 text-sm font-semibold text-primary-foreground transition hover:opacity-95"
            >
          ...
          <button
            onClick={() => printIdCard(t)}
            className="flex w-full items-center justify-center gap-2 rounded-lg border border-border bg-background py-2.5 text-sm font-semibold text-foreground transition hover:bg-secondary"
          >
...
function TradersPage() {
  ...
  // Effective roster reflects payments collected in this session.
  const roster = traders.map((t) => (collected.has(t.id) ? { ...t, paidToday: true } : t));
  ...
  const totalPaid = roster.filter((t) => t.paidToday).length;
  const totalUnpaid = roster.filter((t) => !t.paidToday).length;
  const avgCompliance = Math.round(
    roster.reduce((s, t) => s + t.complianceScore, 0) / roster.length,
  );

  function collect(t: Trader) {
    setPayTarget({
      ref: t.traderId,
      payer: t.name,
      category: `Market Daily Ticket — ${t.marketName}`,
      amount: t.dailyRate,
      meta: [
        { label: "Trade", value: t.tradeType },
        { label: "Category", value: `Cat ${t.category}` },
  ...
  return (
    ...
    >
      {selected && (
        <TraderDrawer t={selected} onClose={() => setSelectedId(null)} onCollect={collect} />
      )}
      <PayDialog
        target={payTarget}
        onClose={() => setPayTarget(null)}
        onPaid={(ref) => {
          const t = traders.find((x) => x.traderId === ref);
          if (t) setCollected((prev) => new Set(prev).add(t.id));
        }}
      />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-secondary/40">
                ...
                <th className="hidden px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground md:table-cell">
                  Trade Type
                </th>
                <th className="hidden px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground lg:table-cell">
                  Market
                </th>
                <th className="hidden px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground lg:table-cell">
                  Assets
                ...
                <th className="hidden px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground sm:table-cell">
                  Today
                </th>
                <th className="hidden px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground xl:table-cell">
                  Score
                </th>
                <th className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-muted-foreground">
                  Ward
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? (
                ...
                filtered.map((t) => (
                  <TraderRow key={t.id} t={t} onClick={() => setSelectedId(t.id)} />
                ))
              )}
            </tbody>
          </table>
