...
function MarketsPage() {
  ...
  const avgCompliance = Math.round(
    traders.reduce((s, t) => s + t.complianceScore, 0) / traders.length,
  );
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 mb-6">
        ...
        />
        <KpiCard
          label="Present Today (est.)"
          value={totalPresent}
          hint="Across 4 markets"
        ...
        <KpiCard
          label="Paid Today"
          value={paidToday}
          hint={`${Math.round((paidToday / totalRegistered) * 100)}% of registered`}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {marketSessions.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)] transition hover:-translate-y-0.5 hover:shadow-[0_4px_24px_-8px_rgba(15,23,42,0.12)]"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${compliancePct >= 80 ? "bg-emerald-50 text-emerald-700" : compliancePct >= 60 ? "bg-amber-50 text-amber-700" : "bg-red-50 text-red-700"}`}
                    >
                      {compliancePct}%
                    </span>
                  </div>
                  ...
                  <div className="mt-3">
                    <div className="mb-1 flex justify-between text-[11px] text-muted-foreground">
                      ...
                      <span>
                        {s.paid}/{s.present} present
                      </span>
                    </div>
                    <div className="h-2 rounded-full bg-secondary">
                      <div
                        className="h-2 rounded-full bg-primary transition-all"
                        style={{ width: `${Math.min(100, (s.paid / s.present) * 100)}%` }}
                      />
                    </div>
                  </div>
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {z.marketName.split(" ")[0]} · {z.traderCount} traders
                      </div>
                    </div>
                    <span
                      className={`shrink-0 rounded-full border px-1.5 py-0.5 text-[9px] font-bold ${catColor}`}
                    >
                      Cat {z.category}
                    </span>
                  </div>
        ...
        <div className="space-y-5">
              ...
              {marketOfficers.map((o) => (
                <div key={o.id} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      {o.marketName.split(" ")[0]} · {o.receiptsToday} receipts
                    </div>
                  </div>
            ...
            <div className="mt-3 rounded-xl border border-border bg-surface px-3 py-2">
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span className="font-bold text-ink">
                  {fmt(marketOfficers.reduce((s, o) => s + o.collectedToday, 0))}
                </span>
              </div>
              <p className="mt-1 text-[10px] text-muted-foreground">
                Every receipt has GPS + officer ID + timestamp — no cash leakage
              </p>
            </div>
          </div>
                ...
                return (
                  <div key={a.id} className="rounded-xl border border-border bg-surface p-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${pct >= 80 ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                      >
                        {pct}%
                      </span>
                    </div>
                    <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
                      <span>
                        {a.registered}/{a.members} registered
                      </span>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-card)]">
            ...
            {(["A", "B", "C"] as const).map((cat) => {
              ...
              const desc =
                cat === "A"
                  ? "Large traders · ₦300/day"
                  : cat === "B"
                    ? "Medium traders · ₦200/day"
                    : "Small/informal · ₦100/day";
              const pct = Math.round((count / traders.length) * 100);
              const color =
                cat === "A" ? "bg-violet-500" : cat === "B" ? "bg-blue-500" : "bg-slate-400";
              const bg =
                cat === "A"
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-amber-700">
                  All collections must have: Receipt No · Officer ID · GPS Location · Timestamp. No
                  receipt = no valid collection.
                </p>
              </div>
            </div>
          </div>
