    ...
    <header className="sticky top-0 z-50 border-b border-border/60 bg-background/85 backdrop-blur-xl supports-[backdrop-filter]:bg-background/70">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        <Link
          ...
          aria-label="Kwali Area Council — home"
        >
          <img src={crest} alt="" className="h-10 w-10 transition group-hover:rotate-[-4deg]" />
          <div className="leading-tight">
            <div className="font-display text-sm font-bold text-primary sm:text-base">
              Kwali Area Council
            </div>
        ...
        <div className="flex items-center gap-2">
          {loading ? null : user ? (
            <>
              <Link
                to="/portal"
                className="hidden rounded-md px-4 py-2 text-sm font-semibold text-primary transition hover:bg-secondary sm:inline-flex"
              >
                My Portal
              </Link>
              {isAdmin && (
            ...
            {!user && (
              <li className="pt-1">
                <Link
                  to="/auth/login"
                  onClick={() => setOpen(false)}
                  className="block rounded-md px-3 py-3 text-primary hover:bg-secondary"
                >
                  Sign in
                </Link>
              </li>
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 md:grid-cols-4">
        <div className="md:col-span-2">
          ...
          <p className="mt-4 max-w-sm text-sm leading-relaxed text-muted-foreground">
            ...
            unifying property, business, transport, market and compliance revenue across the
            council's 10 wards.
          </p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2.5 text-sm text-muted-foreground">
            ...
            <li>
              <Link to="/payments" className="transition hover:text-primary">
                Payments
              </Link>
            </li>
            <li>
              <Link to="/transport" className="transition hover:text-primary">
                Transport tickets
              </Link>
            </li>
            <li>
              <Link to="/compliance" className="transition hover:text-primary">
                Compliance
              </Link>
            </li>
            <li>
              <Link to="/bylaws" className="transition hover:text-primary">
                Bylaws
              </Link>
            </li>
          </ul>
